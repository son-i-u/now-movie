	...
	function timeLimit(limit) {
		...
		function printCard(prefer, dist, i){
			...
			var itemStr = '<div class="card" style="max-width: 540px;" onclick="toDetailPage(' + prefer.movie_id + ');"><div class="row">'
				...
				+ '<p class="card-text">주연:' + prefer.actor + '</p>'
				+ '<p class="card-text">'
				+ '<small class="text-muted">' + prefer.theator_nm + ' ' + '<label id="distance' + i + '">'+ dist +'</label>' + 'km </small></p>'
				+ '<p class="card-text">'
				+ '<small class="text-muted">' + prefer.left_min + '분 뒤 시작 </small></p>'
				+ '</div></div></div></div>';
					...
					geocoder.addressSearch(address,
						function(result, status) {
							...
							if (status === kakao.maps.services.Status.OK) {
								...
								dist = distance(result[0].y, result[0].x, "K");
								console.log("dist: " + dist);
								console.log('#distance' + String(index));
								$('#distance' + String(index)).html("test");
							}
