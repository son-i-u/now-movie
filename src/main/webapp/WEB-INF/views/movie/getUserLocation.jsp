...
<script>
	...
	var callback = function(result, status) {
		...
		if (status === kakao.maps.services.Status.OK) {
			...
			sessionStorage.setItem('user_lat', result[0].x + "");
			sessionStorage.setItem('user_lon', result[0].y + "");

			var position = sessionStorage.getItem('user_lon');
...
<body>
	...
	<form id="location" action="/movie/recommend" method="POST">
		<input type="hidden" id="userLon" name="userLon" value='0'></input> <input
			type="hidden" id="userLat" name="userLat" value='0'></input>
	...
	<script type="text/javascript">
		window.onload = function() {
			setInterval(function() {
				if (document.getElementById('userLon').value != '0'
						&& document.getElementById('userLat') != '0')
